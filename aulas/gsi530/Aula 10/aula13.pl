...
tamanho_lista([], 0).
tamanho_lista([Cabeca|Cauda], N) :- N is 1 + tamanho_lista(Cauda).
...
insereElementoFinal(X, [Z|Cauda], [Z|Cauda2]) :- insereElementoFinal(X, Cauda, Cauda2).

insereElementoInicio(X, [], [X]).
insereElementoInicio(X, [Z|Cauda], [X|Cauda2]) :- insereElementoInicio(Z, Cauda, Cauda2).
